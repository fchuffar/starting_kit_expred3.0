...
alternative_programs = list(
  ...
  program_sis = function(data_train, data_test) {
    ...
    ms = sapply(genes, function(g){
      # print(g)
      formula = as.formula(paste0("ALS2~", g))
    ...
    formula = as.formula(paste0("ALS2~", g))
    m = lm(formula, data_train)
    plot(data_train[[g]], data_train$ALS2, xlab=g, main=paste0("ALS2~", g, " R^2=", signif(summary(m)$r.squared,3)))
  ...
  program_sis2 = function(data_train, data_test) {
    ...
    formula = as.formula(paste0("ALS2~", paste0(gs, collapse="+")))
    m = lm(formula, data_train)
